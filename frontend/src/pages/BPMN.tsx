...
export const BPMN: React.FC = () => {
    ...
    const createdModel = useAppSelector(state => state.bpmn.model, shallowEqual);
    const uploadedModelData = useAppSelector(state => state.bpmn.uploadedData, shallowEqual);
    ...
    const downloadXML = async () => {
        ...
        if(createdModel){
            ...
            downloadFile(xmlContent, `${createdModel.name}.xml`, 'application/xml');
        }
        else{
            ...
            if (createdModel && result) {
                ...
                img.onload = async function () {
                    if (canvas && ctx) {
                        ...
                        picture.src = url;
                        const doc = new jsPDF("l", 'px', [canvas.width, canvas.height], true);

                        picture.onload = function () {
                            doc.addImage(picture, 'PNG', 20, 20, picture.width-40, picture.height-40);
    ...
    const closeDialog = async () => {
        ...
        }else if (typeof newData !=='undefined' && createdModel.data === newData.xml) {
            close();
        } else {
    ...
    const Dialog = () => {
        const modelsInSameDir = useAppSelector(state => state.dirModel.containedModels);
        const usedNamesForModels = Object.values(modelsInSameDir).map(model => model.name);
        ...
        const handleDelete = () => {
            dispatch(dirModelThunk.deleteModel(createdModel ? createdModel.id : -1))
            closeDialog();
        }
        ...
        switch (activeDialog) {
            ...
            case DialogType.BACKHOME:
                return <BackHome closeDialog={close} handleSubmit={handleBackhome} />; // Das Diagramm sollte vor dem Request vergleicht wird

            case DialogType.SPEICHERN:
                return <SaveDialog closeDialog={close} handleSubmit={handleAddNewModel} />;
    ...
    return (
        <div className="bpmn-container">
            ...
            <div className="buttons-bpmn">
                <button className="custom-button green-button" onClick={closeDialog}>Home</button>
                {canEdit && (
                    <>
                        <button className="custom-button green-button" onClick={onClickSave}>
                            {
                        ...
                        {createdModel && (
                            <>
                                <button className="custom-button green-button" onClick={handleRenameButton}>Umbenennen
                                </button>
                                <button className="custom-button green-button" onClick={handleDeleteButton}>Löschen
                                </button>
                                <button className="custom-button green-button" onClick={downloadXML}>XML exportieren
                                </button>
                                <button className="custom-button green-button" onClick={downloadPDF}>PDF
                                    veröffentlichen
                                </button>
                                <button className="custom-button green-button" onClick={downloadSVG}>SVG exportieren
                                </button>
                            </>
                        )}
                    </>
