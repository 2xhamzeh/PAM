...
const bpmnThunk = {
    ...
    >(
        ...
        async (modelId, { getState, rejectWithValue }) => {
            ...
            try {
                const response = await fetch(`/api/v1/models/${modelId}`, {credentials: 'include'})
                if (response.ok)
                    return camelize(await response.json()) as ModelReturnType
                throw new Error
            } catch (error) {
                return rejectWithValue(`Failed to open model "${containedModels[modelId].name}"`)
            }
        }
    ),
    ...
    >(
        "bpmn/addModel",
        async ({ name, description, xmlData }, { getState, rejectWithValue }) => {
    ...
    >(
        ...
        async (xmlData, { getState, rejectWithValue }) => {
            ...
            try {
                ...
                return rejectWithValue(`Failed to update model "${model!.name}"`)
            }
        }
    ),
