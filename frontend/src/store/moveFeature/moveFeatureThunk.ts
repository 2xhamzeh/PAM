...
const moveFeatureThunk = {
  ...
  >('moveFeature/loadCurrentDirectory', ({}, { getState }) => {
    const {
      currentInode,
      path,
      containedModels,
  ...
  >(
    ...
    async (targetInode, { getState, rejectWithValue }) => {
      ...
      const { containedDirectories, destination_inode } =
        getState().moveFeature;

      if (
        loading !== LoadingState.PENDING ||
        destination_inode === targetInode
      ) {
      ...
      try {
        const response = await fetch(
          `${API_URL}/api/v1/directory/${targetInode}`,
          { credentials: 'include' }
        );
        ...
        return rejectWithValue(
          `Failed to open directory "${containedDirectories[targetInode]}" for moving action`
        );
      }
    }
  ),
  ...
  >(
    ...
    async (modelId, { getState, rejectWithValue }) => {
      ...
      try {
        const response = await fetch(
          `${API_URL}/api/v1/models/${modelId}/move?destination_dir_inode=${destination_inode}`,
          {
            method: 'PUT',
            credentials: 'include',
          }
        ...
        return rejectWithValue(
          `Failed to move model ${containedModels[modelId].name}`
        );
      }
    }
  ),
  ...
  >(
    ...
    async (name: string, { getState, rejectWithValue }) => {
      ...
      try {
        const response = await fetch(
          `${API_URL}/api/v1/directory/?src_inode=${destination_inode}&name=${name}`,
          {
            method: 'POST',
            credentials: 'include',
          }
