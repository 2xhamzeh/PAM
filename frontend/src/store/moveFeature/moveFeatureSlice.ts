  ...
  extraReducers: (builder) => {
    builder
      .addCase(
        ...
        (_state, action) => (_state = action.payload)
      )
      .addCase(
        ...
        (state, action) => {
          const {
            ...
            containedDirectories,
            containedModels,
          }: DirectoryReturnType = action.payload;

          let truncatedPath = [...path];
          if (path.length > 3) {
            const firstTwo = path.slice(0, 2);
            const pointX3: DirMetadata = {
              inode: -1,
              parentInode: -1,
              name: '...',
            };
          ...
          state.containedModels = containedModels.reduce(
            (byId: Record<number, ModelMetadata>, model) => {
              byId[model.id] = model;
              return byId;
            },
            {}
          );
          state.containedDirectories = containedDirectories.reduce(
            (byId: Record<number, DirMetadata>, dir) => {
              byId[dir.inode] = dir;
              return byId;
            },
            {}
